...
import 'package:my_needs/firebase/models/actualization.dart';

class ActualizationDatabase {
  String uid;
  CollectionReference actualizationCollection;

  ActualizationDatabase(uid) {
    this.uid = uid;
    this.actualizationCollection = FirebaseFirestore.instance
        .collection('users')
        .doc(uid)
  ...
  Future addUserDailyProgress(
    ...
    return await actualizationCollection.add({
  ...
  Stream<List<Actualization>> getDailyProgressStream() {
    print("This is the getDailyProgressStream");
    return actualizationCollection.snapshots().map((snapShot) => snapShot.docs
        .map((document) => Actualization.fromJson(document.data(), document.id))
        .toList());
  }
/*
